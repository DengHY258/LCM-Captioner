...
class BaseTrainer:
    ...
    def predict_for_evalai(self, dataset_type):
        ...
        with torch.no_grad():
            ...
            #self.parameter_statistics(reporter)#参数量和计算量
            ...
            while reporter.next_dataset():
                ...
                for batch in tqdm(dataloader):
                    ...
                    model_output = self.model(prepared_batch)
                    #print(model_output.keys())#['scores', 'losses', 'metrics']
                    #print(model_output['scores'].shape)#torch.Size([64, 30, 6786])
                    #print(model_output['losses'])#为空{}
                    #print(model_output['metrics'])#为空{}
                    report = Report(prepared_batch, model_output)
    ...
    def parameter_statistics(self,reporter):
        self.writer.write('start counting parameter')
        total = sum([param.nelement() for param in self.model.parameters()])
        self.writer.write("The number of parameter is {} M".format(total/1e6))
        self.writer.write('finish counting parameter')

        self.writer.write('start counting calculation')
        dataloader = reporter.get_dataloader()
        batch = next(iter(dataloader))
        prepared_batch = self.dataset_loader.prepare_batch(batch)

        flops=calc_flops(self.model,prepared_batch)
        # flops, params = profile(self.model, inputs=(prepared_batch,))
        batch_size = self.training_parameters.batch_size

        self.writer.write("The FLOPs of the model is {} M".format(flops/1e6/batch_size))#这里还有除以batch_size
        self.writer.write('finish counting calculation')
...
def calc_flops(model, input):
    ...
    #print('  + Number of FLOPs: %.2fM' % (total_flops / 1e6 / 2))#这里应该除以batch_size
    return total_flops