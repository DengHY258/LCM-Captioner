...
from pythia.modules.orc_Tran import TransformerEncoderBlock
...
class M4C(BaseModel):
    ...
    def _build_mmt(self):
        self.ocr_tran1 = TransformerEncoderBlock(emb_size = 768,drop_p = 0.,forward_expansion = 4,forward_drop_p = 0.)
        self.ocr_tran2 = TransformerEncoderBlock(emb_size = 768,drop_p = 0.,forward_expansion = 4,forward_drop_p = 0.)
...
###以上是AnchorCaptioner所没有的

        ocr_attention = self.ocr_tran1(fwd_results['ocr_mmt_in'])
        fwd_results['ocr_mmt_in_2'] = self.ocr_tran2(ocr_attention)
        ...
        mmt_results = self.mmt(
            ...
            ocr_emb=fwd_results['ocr_mmt_in_2'],
    ...
    def _forward_output(self, sample_list, fwd_results):
        ...
        ocr_mask = fwd_results['ocr_mask']
        ##下面四句代码两个模型的思路是相同的，不同的是ocr_ptr_net的输入
...
class MMT(BertPreTrainedModel):##mmt的结构存在不同
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(self,
        ...
        encoder_inputs = torch.cat(
            [txt_emb, obj_emb, ocr_emb, dec_emb],#这里使用了txt_emb和ocr_emb
            dim=1
        )
        attention_mask = torch.cat(
            [txt_mask, obj_mask, ocr_mask, dec_mask],#这里使用了txt_mask和ocr_mask
            dim=1
        )
        ...
        mmt_seq_output = encoder_outputs[0]#这一步将三部分输入进行分离，思路和AnchorCaptioner一样
