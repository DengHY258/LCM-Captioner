...
from pythia.modules.layers import ClassifierLayer,CrossAttnDeLightModule
...
class M4C(BaseModel):
    ...
    def _build_obj_encoding(self):
        ...
        self.linear_obj_feat_to_mmt_in = nn.Linear(
            self.config.obj.mmt_in_dim, self.config.crossattn.hidden_size
        )

        # object location feature: relative bounding box coordinates (4-dim)
        self.linear_obj_bbox_to_mmt_in = nn.Linear(
            4, self.config.crossattn.hidden_size
        )

        self.obj_feat_layer_norm = BertLayerNorm(self.config.crossattn.hidden_size)
        self.obj_bbox_layer_norm = BertLayerNorm(self.config.crossattn.hidden_size)
    ...
    def _build_ocr_encoding(self):
        ...
        self.linear_ocr_feat_to_mmt_in = nn.Linear(
            self.config.ocr.mmt_in_dim, self.config.crossattn.hidden_size
        )
        ...
        self.ocr_feat_layer_norm = BertLayerNorm(self.config.crossattn.hidden_size)
        self.ocr_bbox_layer_norm = BertLayerNorm(self.config.crossattn.hidden_size)
        self.ocr_drop = nn.Dropout(self.config.ocr.dropout_prob)

    def _build_mmt(self):
    
        self.fuselayer = CrossAttnDeLightModule()
    ...
    def _forward_mmt(self, sample_list, fwd_results):
        ...
        obj_fuse_emb,ocr_fuse_emb = self.fuselayer(fwd_results['obj_mmt_in'],fwd_results['ocr_mmt_in'])
        

        # mmt_results = self.mmt(
        #     txt_emb=fwd_results['txt_emb'],
        #     txt_mask=fwd_results['txt_mask'],
        #     obj_emb=obj_fuse_emb,
        #     obj_mask=fwd_results['obj_mask'],
        #     ocr_emb=ocr_fuse_emb,
...
class MMT(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        encoder_inputs = torch.cat(
            [obj_emb, ocr_emb, dec_emb],#txt_emb,ocr_emb
            dim=1
        )
        attention_mask = torch.cat(
            [obj_mask, ocr_mask, dec_mask],#txt_mask,ocr_mask
            dim=1
        )
